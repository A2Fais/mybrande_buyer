...
export async function saveCanvas(
  ...
  try {
    ...
    if (response?.status === 200) {
      ...
      logoNameObj.set("fontFamily", logoNameFontFamily);
      sloganNameObj.set("fontFamily", sloganNameFontFamily);
      canvas.renderAll();

      const redirectUrl = isPackage
        ? `https://www.mybrande.com/buyer/logo/downloadandpayment/${buyer_logo_id}`
        : `https://www.mybrande.com/user/logo/preview/${buyer_logo_id}`;
    ...
    toastNotification("Error saving logo");
  } finally {
    isSaving = false;
  }
}
